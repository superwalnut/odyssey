...
export class BookingdetailsComponent extends BaseComponent implements OnInit {
  ...
  getBookingPersonAndGroupAdjustment() {
    let bps = this.bookingPersonService.getCustomByBookingDocId(
      this.bookingDocId,
      this.loggedInAccount.docId
    );
    let groupAdjusted =
      this.groupTransactionService.getByBookingDocIdAndPaymentMethod(
        this.bookingDocId,
        GlobalConstants.paymentAdjust
      );
  ...
  getBookingTotal(lbus: LocalBookingUser[]) {
    ...
    lbus.forEach((lbu) => {
      ...
      if (lbu.paymentMethod == GlobalConstants.paymentCredit) {
        ...
        lbu.paymentMethod == GlobalConstants.paymentCash &&
        lbu.isPaid
      ) {
        this.bookingTotalCash += lbu.amount;
      } else if (
        lbu.paymentMethod == GlobalConstants.paymentBank &&
        lbu.isPaid
      ) {
    ...
    if (this.groupTransactionsAdjusted.length > 0) {
      this.bookingTotalAdjusted = this.groupTransactionsAdjusted
  ...
  createBooking(selectedUserControl) {
    ...
    var user = this.allUsersObject.filter((x) => {
    ...
    let bp = {
      ...
      userId: user[0].docId,
      userDisplayName: this.isFriend
        ? "Friend (" + user[0].name + ")"
        : user[0].name,
      avatarUrl: user[0].avatarUrl
        ? user[0].avatarUrl
        : GlobalConstants.imageDefaultAvatar,
      //paymentMethod: this.selectedPaymentMethod, //Credit | Cash | Bank Transfer`
      paymentMethod: this.isParentCreditUser
        ? GlobalConstants.paymentCredit
        : GlobalConstants.paymentCash,
      parentUserId: user[0].parentUserDocId
        ? user[0].parentUserDocId
        : user[0].docId,
      parentUserDisplayName: user[0].parentUserDisplayName
        ? user[0].parentUserDisplayName
        : user[0].name,
      isForSale: false,
      amount: this.getPaymentAmount(
        user[0].docId,
        this.isParentCreditUser,
        this.isFriend
      ),
  ...
  withdrawBooking(lbu: LocalBookingUser) {
    ...
    if (confirm("Are you sure to withdraw? " + lbu.name)) {
      var bp = {
        ...
        parentUserDisplayName: lbu.parentUserDisplayName
          ? lbu.parentUserDisplayName
          : lbu.name,
      } as BookingPerson;
      this.bookingPersonService
        .withdraw(bp.docId, bp, this.booking)
        .catch((err) => {
  ...
  getPaymentAmount(
    ...
    let price = this.helpService.findRates(
      isCreditUser,
      comitteeUser != null,
      isFriend,
      this.group,
  ...
  getPaymentClass(paymentMethod: string) {
    ...
    } else if (paymentMethod == GlobalConstants.paymentBank) {
      return GlobalConstants.bankIcon;
    }
  }

  toggleLockStatus() {
    if (this.booking.reconciled) {
      return false;
    }
  ...
  startReconciliationClicked() {
    ...
    this.bookingService.toggleReconciliationInProgress(
      this.booking,
  ...
  reconciled() {
    if (
      confirm(
        "This action cannot be undone and I confirm this booking is reconciled and to be closed forever!"
      )
    ) {
      this.isLoading = true;
...
export class SeatDialog {
  ...
  onConfirmClick() {
    ...
    this.bookingService
      .updateBooking(this.data.booking.docId, booking)
      .then(() => this.dialogRef.close())
      .catch((err) => {
        alert(err);
      });
