...
export class RptCsvComponent extends BaseComponent implements OnInit {
  ...
  downloadUserCredits() {
    this.creditService
      ...
      .subscribe((x) => {
        ...
        this.snackBar.open(
          `you have successfully download the credit transactions.`,
          null,
          {
            duration: 5000,
      ...
      const report = x.map((g) => {
        return {
          ...
          startDate: g.startDate
            ? this.pipe.transform(g.startDate.toDate(), "short")
            : "",
          endDate: g.endDate
            ? this.pipe.transform(g.endDate.toDate(), "short")
            : "",
          created: this.pipe.transform(g.created.toDate(), "short"),
          createdBy: g.createdBy,
          createdByName: g.createdByDisplayName,
          updated: g.updated
            ? this.pipe.transform(g.updated.toDate(), "short")
            : "",
      ...
      .subscribe((x) => {
        const report = x.map((g) => {
          return {
            ...
            updatedDate: g.updatedOn
              ? this.pipe.transform(g.updatedOn.toDate(), "short")
              : "",
          };
        });
        ...
        console.log(
          "🚀 ~ file: rpt-csv.component.ts ~ line 134 ~ RptCsvComponent ~ this.bookingPersonService.getByGroupId ~ report",
          report
        );
      });
  }
  ...
  downloadBookings() {
    this.bookingsService.getAll().subscribe((x) => {
      const report = x.map((g) => {
        return {
          docId: g.docId,
          eventStartDateTime: g.eventStartDateTime
            ? this.pipe.transform(g.eventStartDateTime.toDate(), "short")
            : "",
          groupDocId: g.groupDocId,
          weekDay: g.weekDay,
        };
  ...
  downloadEventLogs() {
    this.eventLogService.getAllEventLogsNoLimit().subscribe((x) => {
      const report = x.map((g) => {
        return {
          eventCategory: g.eventCategory,
          notes: g.notes,
          createdBy: g.createdByDisplayName,
          createdOn: g.createdOn.toDate(),
        };
      });
      super.downloadFile(report, "eventlogs");
    });
  }
