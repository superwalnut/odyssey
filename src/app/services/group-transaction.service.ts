...
export class GroupTransactionService extends FirestoreBaseService<GroupTransaction> {
  ...
  public getByGroupDocId(groupDocId: string) {
    return this.firestore
      .collection("groupTransactions", (q) =>
        q.where("groupDocId", "==", groupDocId).orderBy("created", "desc")
      )
      .snapshotChanges()
      .pipe(
        map((actions) => {
  ...
  public getByBookingDocId(bookingDocId: string) {
    return this.firestore
      .collection("groupTransactions", (q) =>
        q.where("bookingDocId", "==", bookingDocId).orderBy("created", "desc")
      )
      .snapshotChanges()
      .pipe(
        map((actions) => {
  ...
  public getByBookingDocIdAndPaymentMethod(
    bookingDocId: string,
  ...
  public allocateDividend(
    groupDocId: string,
    groupName: string,
    groupBalance: number,
    netProfit: number,
    dividendNotes: string,
    profitHoldingAccount: User,
    committees: User[],
    operatorUserId: string,
    operatorUserDisplayName: string
  ) {
  ...
  public getBalance(groupDocId: string) {
    return this.firestore
      ...
      .pipe(
        map((actions) => {
          const amounts = actions.map(
            (x) => (x.payload.doc.data() as GroupTransaction).amount
          );
  ...
  public bookingReconciliation(
    ...
    lbus.forEach((lbu) => {
      ...
      if (lbu.paymentMethod == GlobalConstants.paymentCash) {
        credit.note += ": Cash reveivable";
      }
      batch.set(ref, credit);

      if (lbu.paymentMethod == GlobalConstants.paymentCash) {
        //for cash user, we need to credit back to make balance 0
        ...
        var credit = {
          ...
          userDisplayName: lbu.name,
          note:
            group.groupName + ": " + lbu.name + " " + lbu.note ??
            "" + ": Cash on hand",
          createdBy: operator.docId,
          createdByDisplayName: operator.name,
          created: Timestamp.now(),
